...
function formatWorkingHours(workingHours: any): string {
  if (typeof workingHours === 'object' && workingHours !== null) {
    if (workingHours.start && workingHours.end) {
      return `from ${workingHours.start} to ${workingHours.end}`;
    }
  }
      ...
      switch (func) {
        case 'content-calendar':
          return `Develop a strategic and well-structured monthly content calendar for ${inputs.brandName}, a company operating in the ${inputs.industry} industry. 
                  ...
                  Provide weekly post suggestions that are platform-specific, with tailored messaging for ${safeJoin(inputs.platforms)}, reflecting best practices for each channel. 
        ...
        case 'social-post':
          return `Craft a compelling and platform-optimized social media post for ${inputs.brandName}, a brand operating in the ${inputs.industry} industry. 
                  ...
                  Tailor the content for use across the following platforms: ${safeJoin(inputs.platforms)}, ensuring that formatting, tone, and structure fit the norms of each. 
        ...
        case 'story-creation':
          return `Craft a captivating, mobile-first short-form story or ad narrative for ${inputs.brandName}, a business in the ${inputs.industry} sector. 
                  ...
                  Naturally incorporate the following keywords into the story: ${safeJoin(inputs.keywords)}, using them to reinforce the core message and improve discoverability. 
        ...
        case 'ad-copy':
          return `Write high-converting ad copy for ${inputs.brandName}, a brand in the ${inputs.industry} industry, designed to promote the following offering: ${inputs.productDescription}. 
                  ...
                  Integrate these keywords seamlessly into the message to improve relevance and visibility: ${safeJoin(inputs.keywords)}. 
                  Ensure the ad copy matches the norms and best practices of the target platforms: ${safeJoin(inputs.platforms)}, including length, formatting style, and content restrictions. 
        ...
        case 'seo-optimization':
          return `Optimize the following product or service description for ${inputs.brandName}, a business operating in the ${inputs.industry} industry. 
                  ...
                  Ensure the copy is tailored for high performance across digital platforms such as ${safeJoin(inputs.platforms)}, taking into account current SEO best practices. 
    ...
    case 'organization':
      switch (func) {
        case 'daily-checklist':
          return `Create a personalized daily checklist for a ${inputs.role} who works ${formatWorkingHours(inputs.workingHours)}. 
                  The checklist should comprehensively cover their key responsibilities, including ${safeJoin(inputs.responsibilities)}, and incorporate both unique tasks such as ${safeJoin(inputs.tasks)} and recurring tasks like ${safeJoin(inputs.recurringTasks)}. 
                  ...
                  Take into account collaboration opportunities with team members, specifically ${safeJoin(inputs.teamMembers)}, by highlighting tasks that require coordination or handoffs. 
        ...
        case 'weekly-planner':
          return `Create a detailed and well-structured weekly planner for a ${inputs.role} who works ${formatWorkingHours(inputs.workingHours)}. 
                  This individual is accountable for the following key responsibilities: ${safeJoin(inputs.responsibilities)}, 
                  and frequently collaborates with team members such as ${safeJoin(inputs.teamMembers)} to ensure smooth workflow and communication. 
                  The planner should be designed to help accomplish the specified weekly goals: ${safeJoin(inputs.weeklyGoals)}, by breaking them down into daily priorities that are clear and manageable. 
                  Integrate recurring tasks like ${safeJoin(inputs.recurringTasks)} throughout the week to maintain consistency and reinforce routines. 
        ...
        case 'task-delegation':
          return `Design a comprehensive task delegation strategy for a ${inputs.role} responsible for the following key duties: ${safeJoin(inputs.responsibilities)}. 
                  The objective is to efficiently delegate the tasks: ${safeJoin(inputs.tasks)} among the team members: ${safeJoin(inputs.teamMembers)}. 
        ...
        case 'support-workflow':
          return `Create a comprehensive support workflow for a ${inputs.role} who leads a team consisting of: ${safeJoin(inputs.teamMembers)}. 
                  This workflow should effectively manage customer interactions across multiple channels, including: ${safeJoin(inputs.customerChannels)}, ensuring seamless communication. 
                  It must efficiently address common customer issues such as: ${safeJoin(inputs.commonIssues)}, providing clear and actionable solutions. 
        ...
        case 'sop-template':
          return `Generate a comprehensive Standard Operating Procedure (SOP) template for a ${inputs.role} responsible for the following duties: ${safeJoin(inputs.responsibilities)}. 
                  The SOP should clearly outline the steps for recurring tasks, including ${safeJoin(inputs.recurringTasks)}, to ensure consistency and efficiency. 
                  It must also support uniform service delivery across all customer channels such as ${safeJoin(inputs.customerChannels)}. 
      ...
      switch (func) {
        case 'brand-positioning':
          return `Develop a comprehensive brand positioning strategy for ${inputs.businessName} that aligns closely with its mission statement: "${inputs.missionStatement}." 
                  Begin by thoroughly integrating the brand's core values, which include: ${safeJoin(inputs.brandValues)}, to ensure authenticity and consistency across all touchpoints. 
                  Conduct a detailed analysis of key competitors such as ${safeJoin(inputs.competitors)}, highlighting how ${inputs.businessName} differentiates itself within the market. 
        ...
        case 'audience-definition':
          return `Define the core target audience for ${inputs.businessName} based on its product or service described as: "${inputs.productOverview}." 
                  ...
                  Address their primary pain points and challenges, specifically: ${safeJoin(inputs.painPoints)}, and clarify what they seek in a solution to these problems. 
                  Analyze how this audience currently interacts with content and brands, detailing their preferred channels of engagement such as ${safeJoin(inputs.channels)}. 
        ...
        case 'value-proposition':
          return `Craft a clear, concise, and compelling value proposition for ${inputs.businessName} that effectively communicates the unique value of its product or service, described as: "${inputs.productOverview}." 
                  ...
                  Identify how the brand distinctively addresses and solves the key pain points faced by this audience, specifically: ${safeJoin(inputs.painPoints)}. 
                  Highlight the core benefits provided by the product or service, including ${safeJoin(inputs.benefits)}, and explain why these benefits are critically important to the target customers' needs and desires. 
        ...
        case 'pricing-strategy':
          return `Design a comprehensive and strategic pricing plan for ${inputs.businessName}, whose mission is: "${inputs.missionStatement}." 
                  ...
                  and analyze competitor pricing within the market, including offerings from ${safeJoin(inputs.competitors)} priced at ${inputs.competitorPricing}. 
                  Take into account the unique benefits and value propositions that the product delivers, such as ${safeJoin(inputs.benefits)}, as well as the brand's overall market positioning and long-term goals. 
        ...
        case 'growth-plan':
          return `Create a detailed and strategic growth plan for ${inputs.businessName}, which is currently in the "${inputs.stage}" stage of development. 
                  The company's mission is: "${inputs.missionStatement}," and its growth objectives include: ${safeJoin(inputs.goals)}. 
                  Taking into account the available budget of ${inputs.budget}, and the current marketing and sales channels utilized such as ${safeJoin(inputs.channels)}, develop scalable strategies that span product development, marketing initiatives, sales efforts, and partnership opportunities. Outline specific tactics to maximize performance within the allocated budget, leveraging content formats like ${safeJoin(inputs.contentTypes)} to engage the target audience effectively.
        ...
        case 'content-plan':
          return `Develop a comprehensive content strategy for ${inputs.businessName} that aligns with its primary business goals: ${safeJoin(inputs.goals)}. The company is currently in the "${inputs.stage}" stage and actively distributes content through channels including: ${safeJoin(inputs.channels)}. Recommend the most effective content types, such as ${safeJoin(inputs.contentTypes)}, that will resonate with the target audience at this stage of growth and support key objectives. The strategy should prioritize activities that drive brand awareness, audience engagement, lead generation, or customer education, depending on the business's focus. Provide detailed guidance on core content themes and topics that reflect the brand's voice and values. Outline an optimal publishing frequency and calendar to maintain consistency and maximize audience reach. Suggest storytelling techniques and formats that enhance authenticity, emotional connection, and message clarity. Include recommendations for performance tracking and key metrics to measure the effectiveness of the content efforts. The final content plan should be actionable and scalable, enabling the marketing team to adapt and evolve it as the business grows. Ensure that the strategy fosters long-term brand loyalty and supports cross-channel integration for maximum impact.`;
      }
      break;
  }
    ...
    switch (func) {
      ...
      case 'Story-Creation':
        return `Design a high-impact, mobile-first story sequence for ${inputs.brandName}, crafted specifically for Instagram or Facebook Stories. 
                ...
                Integrate keywords like ${safeJoin(inputs.keywords)} into the design as visual motifs, animated overlays, or stylized text to enhance thematic consistency. 
