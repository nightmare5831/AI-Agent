...
export async function POST(req: Request) {
  try {
    ...
    const system_prompt = `You are a professional education consultant who creates assessment questions for students. You provide detailed, practical and engaging assessment questions that follow modern educational principles. 

IMPORTANT: You must output ONLY a valid JSON format with the following structure:
{
  "title": "Assessment Title",
  "questions": [
    {
      "id": 1,
      "type": "multiple_choice|fill_blank|rewriting|matching|reordering|listening|reading|writing|speaking",
      "question": "Question text in ${targetLanguage}",
      "options": ["option1", "option2", "option3"], // For multiple choice or matching
      "correctAnswer": "Correct answer", // Or correct index for multiple choice
      "points": number,
      "instructions": "Instructions for this question in ${targetLanguage}" // Optional
    },
    // More questions...
  ],
  "totalPoints": number,
...
The assessment should be created in ${targetLanguage}. Make sure the JSON is valid and properly formatted. All text content (questions, options, instructions) should be in ${targetLanguage}.`;
    let user_prompt = '';
    let sub_prompt = '';
    let audio_prompt = '';
    let reading_prompt = '';
...
Ability to produce coherent and structured texts using the full range of organizational models and linking structures appropriately and completely.`;
    ...
    //===============================================================================================
    if (body.assessment_type.evaluation === 'Linguistic') {
      ...
      - Multiple-choice questions (MCQs) (three answer choices). Create ${body.grading_system.questions_number} questions in different formats.
      - Fill-in-the-blank exercises (words to complete). Create ${body.grading_system.questions_number} questions in different formats.
      - Sentence or text rewriting/transformation (grammatical or lexical modification). Create ${body.grading_system.questions_number} questions in different formats.
      - Matching exercises (linking words/definitions or expressions/meanings). Create ${body.grading_system.questions_number} questions in different formats.
      - Sentence reordering exercises (placing words in the correct order). Create ${body.grading_system.questions_number} questions in different formats.
The exercises should assess grammar, conjugation, syntax, and vocabulary while ensuring coherence with the pedagogical objective and the targeted level.`;
    ...
    if (body.assessment_type.evaluation === 'Skills Based') {
...
- Listening comprehension: ${audio_prompt} Create ${body.grading_system.questions_number} questions in different formats.
- Reading comprehension: ${reading_prompt} Create ${body.grading_system.questions_number} questions in different formats.
- Writing production: ${writing_prompt} Create ${body.grading_system.questions_number} questions in different formats.
- Speaking production: ${speaking_prompt} The phrase "in front of your teacher" must be included. Create ${body.grading_system.questions_number} questions in different formats. And the speaking production task must be either one-on-one with the teacher or recorded and submitted as an audio file.
Please strictly follow the following rules:
${rule_1}
`;
    }
    if (body.assessment_type.evaluation === 'Mixed') {
...
${rule_1}.`;
    ...
    user_prompt = `Create assessment questions for your students based on the following details:
...
- Assessment Type: ${body.assessment_type.type}
- Evaluation Type: ${body.assessment_type.evaluation}
- Important Information: ${body.content.important_info}
- Assessment Content: ${body.content.assessment_content}
- Number of Questions: ${body.grading_system.questions_number}
- Total Points: ${body.grading_system.total_points}
4. Requirements:
 - ${sub_prompt}
- Ensure that the grading scale is consistent with the difficulty of the questions and correctly adds up to the total number of points indicated above.
- Make the assessment adaptable for individual student levels.
Add:
