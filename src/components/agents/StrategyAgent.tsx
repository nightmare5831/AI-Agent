...
export const StrategyAgent = () => {
  ...
  const validateForm = () => {
    ...
    const getRequiredFields = () => {
      switch (selectedFunctionality) {
        ...
        case 'audience-definition':
          return ['businessName', 'productOverview', 'audienceDescription', 'painPoints', 'channels'];
        case 'value-proposition':
          return ['businessName', 'productOverview', 'audienceDescription', 'painPoints', 'benefits'];
        case 'pricing-strategy':
          return ['businessName', 'baseCost', 'competitorPricing', 'benefits', 'competitors', 'missionStatement'];
        case 'growth-plan':
          return ['businessName', 'stage', 'goals', 'budget', 'channels', 'missionStatement', 'contentTypes'];
        case 'content-plan':
    ...
    requiredFields.forEach(field => {
      if (['brandValues', 'competitors', 'painPoints', 'benefits', 'channels', 'contentTypes'].includes(field)) {
  ...
  const renderFields = () => {
    ...
    if (['brand-positioning', 'pricing-strategy', 'growth-plan'].includes(selectedFunctionality)) {
      fields.push(
    ...
    if (['brand-positioning', 'pricing-strategy'].includes(selectedFunctionality)) {
      fields.push(
        <div key="competitors" className="space-y-3">
          ...
          />
          {errors.competitors && <p className="text-sm text-red-500">{errors.competitors}</p>}
        </div>
      );
    }

    // Product Overview (for audience-definition, value-proposition)
    if (['audience-definition', 'value-proposition'].includes(selectedFunctionality)) {
      fields.push(
        <div key="productOverview" className="space-y-2">
          ...
          />
          {errors.productOverview && <p className="text-sm text-red-500">{errors.productOverview}</p>}
        </div>
      );
    }
    ...
    if (['audience-definition', 'value-proposition'].includes(selectedFunctionality)) {
      fields.push(
        <div key="audienceDescription" className="space-y-2">
          ...
          />
          {errors.audienceDescription && <p className="text-sm text-red-500">{errors.audienceDescription}</p>}
        </div>
      );
    }
    ...
    if (['value-proposition', 'pricing-strategy'].includes(selectedFunctionality)) {
      fields.push(
    ...
    if (selectedFunctionality === 'pricing-strategy') {
      fields.push(
        <div key="pricing" className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => {
                setFormData(prev => ({ ...prev, competitorPricing: e.target.value }));
                clearFieldError('competitorPricing');
              }}
              placeholder="120"
              className={`h-11 ${errors.competitorPricing ? 'border-red-500' : ''}`}
            />
            {errors.competitorPricing && <p className="text-sm text-red-500">{errors.competitorPricing}</p>}
          </div>
        </div>
      );
    }
    ...
    if (selectedFunctionality === 'growth-plan') {
      fields.push(
        <div key="growthFields" className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Select value={formData.stage} onValueChange={(value) => {
              ...
              <SelectTrigger className={`h-11 ${errors.stage ? 'border-red-500' : ''}`}>
    ...
    if (['audience-definition', 'growth-plan', 'content-plan'].includes(selectedFunctionality)) {
      fields.push(
    ...
    if (['growth-plan', 'content-plan'].includes(selectedFunctionality)) {
      fields.push(
        <div key="contentTypes" className="space-y-3">
          ...
          {errors.contentTypes && <p className="text-sm text-red-500">{errors.contentTypes}</p>}
        </div>
      );
    }
